...
(function ($) {
    ...
    $(window).scroll(function () {
        if ($(this).scrollTop()) {
            ...
            $('.fixed-top').removeClass('bg-light').css('top', $('.top-bar').height());
        }
    });
...
$('.intro-box').css({'height':h+'px'});
  ...
  if($(window).width() > 768){
    if($('.about-section .gallery').outerHeight() + $('.about-section .slogan-wrapper').outerHeight() !=  $('.about-section').outerHeight()){
...
jQuery(function($){
        ...
        if(intro_margin > 0){
         ...
         if($('.intro-box .inner').is(':visible')) {
          ...
          var hero_inner_margin = (($('.hero').height() - $('.hero .wrapper .inner').height()) / 2);
        ...
        $("a[data-scrollto]").click(function(e){
           ...
           var destination = $('.' + $(this).data('scrollto'));
            ...
            $('html,body').animate({scrollTop: destination.offset().top },1200, function(){
        ...
        function change_quantity(e, elem, quantity){
          ...
          $.ajax({
              ...
              success: function(data, element) { 
                  ...
                  $(this.element).parents('.cart-items').find('.total .amount span').html(data.total);
        ...
        $('body').on('click','.cart-items .floater .icon, .floater .close-cart', function(e){
             e.preventDefault();
        ...
        $('#dynamic_content').on('click', '.product-detail .instructions a' ,function(e){
            e.preventDefault();
        ...
        $('#dynamic_content').on('click','.product-detail .instructions .button', function(e){
            e.preventDefault();
        ...
        $('body').on('click', '.datepicker-main-wrapper .datepicker-nav.month .next', function(e){
          e.preventDefault();
          $('.ui-datepicker-month option:selected').next().attr('selected', 'selected').change();
        });
        $('body').on('click', '.datepicker-main-wrapper .datepicker-nav.month .prev', function(e){
          e.preventDefault();
          $('.ui-datepicker-month option:selected').prev().attr('selected', 'selected').change();
        });

        $('body').on('click', '.datepicker-main-wrapper .datepicker-nav.year .next', function(e){
          e.preventDefault();
          $('.ui-datepicker-year option:selected').next().attr('selected', 'selected').change();
          
        });

        $('body').on('click', '.datepicker-main-wrapper .datepicker-nav.year .prev', function(e){
          e.preventDefault();
          $('.ui-datepicker-year option:selected').prev().attr('selected', 'selected').change();
        });


        $('.become-client .datepicker-main-wrapper').on('click', '.set-date', function(e){

          e.preventDefault();
           var dateAsObject = $( ".become-client .datepicker_selector" ).datepicker( 'getDate' ); //the getDate method
           dateAsObject = $.datepicker.formatDate('MM dd, yy', new Date(dateAsObject));
           $('#pickup_date').val(dateAsObject);
           $('#pickup_time').val($('.become-client .ui-timepicker-select option:selected').html());

           $('.selected-date .date').html(dateAsObject);
           $('.selected-date .time').html($('.ui-timepicker-select option:selected').html());
        ...
        $('body').on('click', '.checkout .set-date', function(e){
          ...
          var dateAsObject = $(this).parents('.checkout').find('.datepicker_selector').datepicker( 'getDate' ); //the getDate method
          pickup_date = $.datepicker.formatDate('MM dd, yy', new Date(dateAsObject));
          pickup_time = $(this).parents('.checkout').find('.ui-timepicker-select option:selected').html();
        ...
        $('.datepicker-main-wrapper').on('click', '.become-client .calendar .submit', function(e){
            $('.hloading').show(); 
            e.preventDefault();
            var dateAsObject = $( ".datepicker_selector" ).datepicker( 'getDate' ); //the getDate method
            pickup_date = $.datepicker.formatDate('MM dd, yy', new Date(dateAsObject));
            pickup_time = $('.become-client .ui-timepicker-select option:selected').html();
        ...
        $('.about-section .gallery,.about-section .slogan-wrapper, .about-section .about').mouseenter(function(e) {
...
function get_products(elem, category){
  $('.hloading').show();
    ...
    $.ajax({
       ...
       $('#dynamic_content').hide().html(data).show("slide", { direction: "left" }, 300);
       if($(window).width() <= 768){
          var mobile_menu_height = $('.mobile_menu').outerHeight();
       		$('html,body').animate({scrollTop: $('#product-list').offset().top - mobile_menu_height },1);
       }else{
...
function get_product_detail(elem, productid){
  ...
  $.ajax({
     ...
     $('#dynamic_content').html('').hide().append(data).show("slide", { direction: "left" }, 300, function(){
        $('#dynamic_content').css('height','auto');
          if(!$('.intro-box .inner').is(':visible')) {
            if(!$('.mobile_menu').is(':visible') || $(window).width()>540 ) {
              $('html,body').animate({scrollTop: $('.product-detail').offset().top },500);
            }else{
              var scrolltop = $('.product-detail').offset().top - $('.mobile_menu').outerHeight();
...
jQuery(document).ready(function($){
    ...
    $('#dynamic_content').on('click','.product-detail .buy-button', function(e){
      $('.hloading').show(); 
      e.preventDefault();
      var url = window.location.hash.substring(2);
      var urlsplit = url.split("/");
      ...
      if(productid){
        ...
        var special_instructions = $('.product-detail #special_instructions').val();
        var size_id = $('.product-detail .size_id_field').val();
      $.ajax({
        url: site_url+'set/buy/',
        data: {product_id : productid, size_id: size_id, quantity: quantity, decoration_id: decoration, special_instructions : special_instructions},
        type: 'POST',
      }).done(function(data) {
         if(data=='1'){
          update_cart();
    ...
    $('body').on('click', '#payment_modal_logged .payment_info .submit', function(e){
        e.preventDefault();
    ...
    $('body').on('click','.cart-items .item .remove-item, .checkout .item .remove-item', function(e){
        e.preventDefault();
        ...
        if(productid){
          ...
          $.ajax({
              ...
              element: elem,
               dataType: 'json',
              success: function(data,element) {
                      $(this.element).closest('.item').fadeOut('slow', function(){
                      ...
                      if(data.total_items == '0'){   
                        ...
                        $('.cart-items .total .amount span, .checkout .total .amount span').html(data.total);
    ...
    $('.intro-box .button.closeme').click(function(e){
      e.preventDefault();
      $('.intro-box, .intro-box-bg').fadeOut(500, function() { $(this).remove()});
			$('#menu-icon').fadeIn();
    });
    ...
    $('.intro-box .button.submit').click(function(e){
        ...
        $.ajax({
          ...
          data: {language : language, business_type: business_type, specify_business: 1},
          type: 'POST',
        }).done(function(data) {
           if(language){
              location.reload();
    ...
    $('body').on('click', '#checkout_wholesale_modal .apply .submit', function(e) {
        e.preventDefault();
        var email =  $('#checkout_wholesale_modal .apply input[type="text"]').val();
    ...
    $('body').on('click', '#checkout_wholesale_modal .forgot .submit', function(e) {
        e.preventDefault();
        var email =  $('#checkout_wholesale_modal .forgot input[type="text"]').val();
        ...
        return false;
         ...
         $('.hloading').show(); 
            $.ajax({
              ...
              data: {first_name : first_name, last_name: last_name, email_address: email_address, phone_number: phone_number, pickup_time: pickup_time, pickup_date: pickup_date},
              type: 'POST',
            }).done(function(data) {
               $('.hloading').hide();
               $('.modal').modal('hide'); 
